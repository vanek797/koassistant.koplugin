...
- **AI Book Tools (Experimental)**: Let the AI search and read the open book on demand to ground its answers in the actual text. See [AI Book Tools](#ai-book-tools-experimental). Supported on Gemini, Claude (Anthropic), OpenAI (API or Subscription), OpenRouter (Claude/GPT/Gemini models), plus DeepSeek, Mistral, Groq, xAI, Fireworks, Qwen (`qwen3-max`), Kimi (`kimi-k2.6`), NVIDIA, OpenCode Zen and OpenCode Go (tool-capable models only), and Ollama; other providers fall back to normal chat. Requires "Allow Text Extraction".
  ...
  - **AI Book Tools: Read Short Texts Whole**: On by default. When the readable text (up to your position while spoiler protection is on) is short enough, the AI gets it in one piece instead of searching it: about 32,000 characters at Quick effort, 64,000 at Standard, 128,000 at Thorough. A novella, a short story or the first chapters of a long book are then covered in full. Off: always search.
