...
local function loadOne(path, identity)
    ...
    if ok_json and type(json) == "table" and type(json.decode) == "function" then
...
local function buildIdentityBase(opts, before, util)
    ...
    if not ok_format or not exactString(format, 128) or not ok_dom or not finiteInteger(dom)
        or not ok_open_md5 or not exactString(open_md5, 128)
        or not ok_disk_md5 or disk_md5 ~= open_md5
        or type(open_modification) ~= "number" or open_modification ~= open_modification
        or open_modification == math.huge or open_modification == -math.huge
        or open_modification ~= before.modification
        or not opened_dom or current_dom ~= opened_dom
        or not ok_revision or not exactString(revision, 256) then return nil end
    return {
...
function Index.start(opts)
    ...
    local path = resolvePath(opts.file)
    local before = fileAttributes(opts.file)
    if not path or not before then return nil, "disabled" end
    ...
    if not ok_util or type(util) ~= "table" or type(util.writeToFile) ~= "function"
        ...
        or type(os.rename) ~= "function" then return nil, "unsupported" end
    local identity = buildIdentityBase(opts, before, util)
    if not identity then return nil, "disabled" end
    local ok_json, json = pcall(require, "json")
    if not ok_json or type(json) ~= "table" or type(json.decode) ~= "function"
        or type(json.encode) ~= "function" then return nil, "unsupported" end
    ...
    if not ok_feeder or type(feeder) ~= "function" then return nil, "unsupported" end
    local handle = io.open(opts.file, "rb")
    if not handle then return nil, "disabled" end
    local state = {
...
function Index.flush(state)
    ...
    if ok_json and type(json) == "table" and type(json.encode) == "function" then
